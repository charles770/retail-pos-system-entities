...
export class ClienteService {
  ...
  protected convertDateFromClient(cliente: ICliente): ICliente {
    const copy: ICliente = Object.assign({}, cliente, {
      fechaIngreso: cliente.fechaIngreso != null && cliente.fechaIngreso.isValid() ? cliente.fechaIngreso.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((cliente: ICliente) => {
        cliente.fechaIngreso = cliente.fechaIngreso != null ? moment(cliente.fechaIngreso) : null;
      });
    }
    return res;
  }
